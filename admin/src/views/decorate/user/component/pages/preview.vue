<template>
    <el-scrollbar class="pages-preview-container">
        ...
        <div
            class="shadow mx-[30px] pages-preview"
            :style="[bodyStyle]"
        >

            <div class="pages-preview-header">
                <div class="phone-title text-lg font-medium">
                    <span v-if="meta.title_type == 1">
                        {{ meta?.title || '页面' }}
                    </span>
                    <img v-else-if="meta.title_type == 2" :src="getImageUrl(meta?.title_img)" class="h-[20px]">
                </div>
            </div>
            <div
...
import { ArrowDownBold, ArrowUpBold, Hide, View, CopyDocument, Delete } from '@element-plus/icons-vue'
...
const bodyStyle = computed(() => {
    ...
    switch (bg_type) {
        ...
        case '2':
            return { 'background-image': `url(${getImageUrl(bg_image)})` }
        default:
            return {}
    }
})
