<template>
    <div class="decoration-pages min-w-[1100px]">
        <div class="flex flex-1 h-full justify-between">
            ...
            <attr-setting
                class="w-[450px] scroll-view-content"
...
<script lang="ts" setup name="decorationStaffPages">
...
> = reactive({
    ...
    [pagesTypeEnum.USER]: {
        ...
        name: '个人中心',
        pageMeta: generatePageData(['page-meta']),
        pageData: generatePageData(['user-info', 'user-service'])
    }
})

const activeMenu = ref<string>('5')
...
const getData = async () => {
    ...
    menus[String(data.id)].pageData = data?.data ? JSON.parse(data.data) : null
