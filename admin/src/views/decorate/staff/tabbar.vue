<template>
    <div class="decoration-tabbar">
        ...
        <footer-btns :fixed="true" v-perms="['decorate:tabbar:save']">
            <el-button source="primary" @click="setData">保存</el-button>
        </footer-btns>
    </div>
</template>
<script lang="ts" setup name="decorationStaffTabbar">
import { markRaw } from 'vue'
import { getDecorateTabbar, setDecorateTabbar } from '@/api/decorate'
...
const getData = async () => {
    const data = await getDecorateTabbar({
        source: activeMenu.value
    })
    menus[activeMenu.value].data = JSON.parse(data.data)
}
const setData = async () => {
    await setDecorateTabbar({
        source: currentTab.value.source,
