...
<script lang="ts" setup name="settingUserLogin">
import type { LoginSetup } from '@/api/setting/user'
import { apiLoginConfigGet, apiLoginConfigSet } from '@/api/setting/user'
    ...
    login_way: [
        {
            ...
            validator: (rule: any, value: any, callback: any) => {
                const login_way = formData.login_way.join('').length
                if (login_way === 0) {
...
const getData = async () => {
    try {
        const data = await apiLoginConfigGet()
...
const handleSubmit = async () => {
    const login_way = formData.login_way.map(Number)
    await formRef.value?.validate()
    try {
        await apiLoginConfigSet({ ...formData })
        getData()
    } catch (error) {
