...
<script setup lang="ts" name="settingSearch">
import { getSearch, setSearch } from '@/api/setting/search'
import type { Search, List } from '@/api/setting/search'
...
const list = computed(() => {
    return formData.data.filter((text: List) => text.name).sort((v1, v2) => v2.sort - v1.sort)
})
